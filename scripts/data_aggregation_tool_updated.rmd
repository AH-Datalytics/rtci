...
cdf_all <- read_csv("C:/OneDrive/OneDrive - ahdatalytics.com/Clients/Real Time Crime Index/Open Source Data/_Ben and Jeff work/Months/2026/March 2026 Final.csv",
...
df_w_ref<-df_w_ref %>% select(-State.y) %>% mutate(State = State.x) %>% select(-State.x)
```
...
sample_city_ids_sc <- read_csv("../data/sample_cities.csv", show_col_types = FALSE) %>%
  pull(city_state_id)
