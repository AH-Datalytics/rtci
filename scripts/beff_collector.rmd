...
cdf_vcpc <- cdf_vcpc %>% select(city_state, vio_crime, prop_crime)
```
join to main dataframe
```{r}
cdf1 <- merge(cdf, cdf_vcpc, by = "city_state", all.x = TRUE)
```
...
state_counts$`Agency Name` <- "Sampled Agencies"

#join state counts with agency names to sample
cdf3 <- bind_rows(cdf2, state_counts)
head(cdf3)
```
