...
state_value <- "Rhode Island"

# Add the "State" column to the data frame
final_df$State <- state_value
```
drop some agencies that don't match our list
PROVIDENCE
WARWICK
CRANSTON
PAWTUCKET
...
allowed_cities <- c("PROVIDENCE", "WARWICK", "CRANSTON", "PAWTUCKET")
...
contains_city_name <- function(agency_name, cities) {
  for (city in cities) {
    if (grepl(paste0("^", city, "$"), agency_name, ignore.case = TRUE)) {
      return(TRUE)
    }
  }
  return(FALSE)
...
#
fdf1 <- final_df[sapply(final_df$`Agency Name`, contains_city_name, cities = allowed_cities), ]
...
folder_path <- "C:\\OneDrive\\OneDrive - ahdatalytics.com\\Clients\\Real Time Crime Index\\Open Source Data\\Rhode Island\\Formatted Data"

# Create the full file path
file_path <- file.path(folder_path, "ri_jan22_present.csv")

# Write the data frame to a .csv file
write.csv(fdf1, file = file_path, row.names = FALSE)
```
