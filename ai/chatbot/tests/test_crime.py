...
class TestCrimeApi(TestCommonAdapter):
    ...
    async def test_categories(self):
        ...
        response = await bot.resolve_categories(QueryRequest(query=f"How many murders and robberies in Boston, MA this past year?"))
        self.assertIsNotNone(response)
        self.assertTrue(len(response) == 2)
        response = await bot.resolve_categories(QueryRequest(query=f"How many carjackings in New Orleans this past year?"))
        self.assertIsNotNone(response)
