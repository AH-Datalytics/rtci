...
cdf_all <- read_csv("C:/OneDrive/OneDrive - ahdatalytics.com/Clients/Real Time Crime Index/Open Source Data/_Ben and Jeff work/Months/2026/February 2026 Final.csv",
...
# Fill any remaining NA population_1 values via direct ORI -> CDE lookup.
# Agencies whose city_state_id doesn't resolve cleanly through ref_df1 arrive
# here with population_1 = NA and would otherwise contribute nothing to
# nationwide_counts. This join uses ori.x (the crime-data ORI) as the key.
cde_pop_direct <- read.csv("https://raw.githubusercontent.com/AH-Datalytics/cde-population/master/cde_populations.csv") %>%
  mutate(ori = toupper(ori)) %>%
  rename(cde_pop_direct = population) %>%
  select(ori, cde_pop_direct)

cdf_vcpc_nats <- cdf_vcpc_nats %>%
  left_join(cde_pop_direct, by = c("ori.x" = "ori")) %>%
  mutate(population_1 = coalesce(as.numeric(cde_pop_direct), as.numeric(population_1))) %>%
  select(-cde_pop_direct)
...
```
override nationwide population from cde population_1
```{r}
# Compute CDE-based nationwide population: sum of distinct agency CDE populations
cde_national_pop <- cdf_vcpc_nats %>%
  group_by(city_state_id) %>%
  slice(1) %>%
  ungroup() %>%
  summarise(total = sum(as.numeric(population_1), na.rm = TRUE)) %>%
  pull(total)

message("CDE national population total: ", cde_national_pop)

# Apply directly to all Nationwide Count rows in the output
cleaned_data <- cleaned_data %>%
  mutate(population = if_else(`Agency Name` == "Nationwide Count", cde_national_pop, population))
```
