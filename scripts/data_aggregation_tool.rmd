...
ref_pop <- read.csv("C:\\OneDrive\\OneDrive - ahdatalytics.com\\Clients\\Real Time Crime Index\\Open Source Data\\FBI Agency Participation Data\\2023 Participation.csv")

#make a city_state column for matching
ref_df <- ref_df %>% mutate(city_state = paste(Agency, State, sep = ", "))
ref_pop <- ref_pop %>% 
              select(pub_agency_name, state_abbr, population) %>%
              mutate(city_state = paste(pub_agency_name, state_abbr, sep = ", ")) %>%
              rename(pop23 = population) %>%
              filter(!is.na(pop23) & pop23 >0) 

#merge the new figures 
ref_df1 <- merge(ref_df, ref_pop, by = "city_state", all.x = TRUE)

#slim it down for now
ref_df1 <- ref_df %>% select(city_state, Population, pop23)
