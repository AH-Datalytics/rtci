...
ref_df1 <- ref_df1 %>% select(-Agency_Type.x, -Agency_Type.y, -State)
#write.csv(failed_merges, "failed_merges.csv", row.names = FALSE)
```
# Add population and region data merging by city/state col
```{r}
cdf_vcpc_nats <- merge(cdf_vcpc_nats, ref_df1, by = c("city_state"), all.x = TRUE)
cdf_vcpc_nats <- cdf_vcpc_nats %>% mutate(State_ref = State) %>%   select(-any_of(c("State.x", "State.y")))
