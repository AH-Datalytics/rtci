...
cdf_vcpc_nats_states <- cdf_vcpc_nats %>%
  mutate(`Violent Crime` = vio_crime,
          ...
          Agency_num =1) %>%
           select(-vio_crime, -prop_crime)
```
check state and region values for correct number of distinct entities
```{r}
# Calculate the number of unique values in the city_state column
num_unique_city_state <- cdf_vcpc_nats_states %>%
...
#
num_unique_city_regions <- cdf_vcpc_nats_regions %>%
  summarise(unique_count = n_distinct(Region)) %>%
  pull(unique_count)

# View the result
print(num_unique_city_regions)
```
