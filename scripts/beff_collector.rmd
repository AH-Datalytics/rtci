...
```{r}
# Apply the function to each dataframe in the list and store the results
unique_agency_counts <- map(ldf1, get_unique_agency_count)
...
```{r}
# Apply the function to each dataframe in the list and store the results
unique_agency_counts <- map(ldf2, get_unique_agency_count)
...
```{r}
# Apply the function to each dataframe in the list and store the results
unique_agency_counts <- map(ldf3, get_unique_agency_count)
...
unique_agencies <- extract_unique_agencies(df_list, "Agency Name")

# Create a dataframe with these unique values
df1 <- tibble(Agency Name = unique_agencies)
...
unique_values_df2 <- extract_unique_values(df2, "Agency.Name")
