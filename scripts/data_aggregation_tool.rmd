...
let's cleave off everything that is extra
...
undesirables_na_historical <- cdf_recent %>%
  mutate(across(where(is.numeric), ~ ifelse(is.na(.), NA, .))) %>%  # Ensure numeric missing values are NA
  filter(Year <= two_months_previous_year &
         Month < two_months_previous_month &
         is.na(Murder))
...
unique_counts_by_agency_type <- cdf_vcpc_nats1 %>%
...
join ref to sample
```{r}
  df_w_ref <- df_mvs_12mo1 %>% select(-Agency_Type.y) %>% mutate(Agency_Type = Agency_Type.x) %>%
  left_join(ref_df1 %>% distinct(city_state, Agency_Type, .keep_all = TRUE), by = c("city_state", "Agency_Type"))
...
sample_cities_viz <- sample_cities %>% 
  ...
  select(city_state, `Agency Name`, State, date, Agency_Type)
