...
```
create a separate df that will be city + , + state and then run the length to get the full number of agencies 
```{r}
...
how many are we down to if i remove everything with an NA in murder?
```{r}
# 1. Identify observations with NA in the "Murder" column
na_murder_data <- na_two_months_previous %>% 
  filter(is.na(Murder))

# 2. Generate a list of unique agency names with NA in the "Murder" column
agencies_with_na_murder <- unique(na_murder_data$`Agency Name`)

# Print the list of unique agencies with NA in "Murder"
print(agencies_with_na_murder)

# 3. Remove all records associated with these agencies from the full dataset
df_mvs_12mo2 <- df_mvs_12mo1 %>% 
  filter(!`Agency Name` %in% agencies_with_na_murder)
```
...
write.csv(df_mvs_12mo1, file = file_path, row.names = FALSE)

# Write the data frame to the data folder in Github repo
write.csv(df_mvs_12mo1, "data/rtci_benjeff_sample.csv")
```
final agency numbers?
```{r}
...
# Create the full file path
file_path <- file.path(folder_path, "non_reporters_60days.csv")

# Write the data frame to a .csv file
write.csv(na_two_months_previous, file = file_path, row.names = FALSE)
```
