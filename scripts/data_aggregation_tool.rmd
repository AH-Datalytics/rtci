...
```{r}
#cdf_all <- bind_rows(cdf, counties_cdf)
```
check sample cities
```{r}
library(readr)
...
# Define file path
file_path <- "C:/OneDrive/OneDrive - ahdatalytics.com/Clients/Real Time Crime Index/Open Source Data/_Ben and Jeff work/nov_24_agencies.csv"
...
# View first few rows
head(df)
# Merge by a common column (e.g., "city_state")
merged_df <- left_join(df, sample_cities, by = "city_state")

# View result
head(merged_df)

# Find records in sample_cities that are not in df
dropped_records <- anti_join(sample_cities, df, by = "city_state")

# View the dropped records
head(dropped_records)
```
...
```{r}
num_unique_A <- n_distinct(cdf_nwcounts$city_state)
print(num_unique_A)
```
vc/pc groupings
```{r}
...
ref_df1 <- ref_df %>% select(city_state, pop23, Agency_Type)%>%
...
```
check to make sure the crosswalk worked part 2
```{r}
# Find rows in ref_df1 whose city_state is not in ref_df2
diff_1 <- ref_df1 %>% filter(!(city_state %in% ref_df2$city_state))

# Find rows in ref_df2 whose city_state is not in ref_df1
diff_2 <- ref_df2 %>% filter(!(city_state %in% ref_df1$city_state))

# Combine the differences
diff_records <- bind_rows(diff_1, diff_2)

# View the differing records
print(diff_records)
```
# Add population and region data merging by city/state col
```{r}
cdf_vcpc_nats <- merge(cdf_vcpc_nats, ref_df2, by = c("city_state"), all.x = TRUE)
```
...
nationwide_counts <- cdf_vcpc_nats_usa %>%
  ...
  summarise(
    ...
    Agency_num = n_distinct(city_state),  # Count unique agencies per month
    .groups = 'drop'
  )
...
df_w_ref <- df_mvs_12mo1 %>%
  left_join(ref_df2, by = "city_state")
```
...
final_data <- df_w_ref %>%
  rename(
    #State = State.x,
    #State_ref = State.y,
    Agency_Type = Agency_Type.y
  ) %>%
  select(-Agency_Type.y.y)
```
...
# Create the full file path for "final_sample.csv" to store on sharepoint
...
# Write a copy of the sample in the data archive folder as well with suffix
write.csv(cleaned_data,file.path("data/archive", paste0("final_sample_", month_suffix, ".csv")), row.names = FALSE)
```
