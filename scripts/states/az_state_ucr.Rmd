...
```{r}
# Function to keep only columns that match names from a list
keep_valid_columns <- function(df, col_names) {
  df %>%
    select(any_of(col_names))
}

# Function to transform a single dataframe
transform_df <- function(df) {
  df %>%
    keep_valid_columns(col_names) %>%
    mutate(across(-1, convert_to_numeric))
}

# Apply the transformation to all dataframes in the list
ldf2 <- lapply(ldf1, transform_df)
