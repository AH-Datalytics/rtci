...
# update with new month folder at the end
data_dir <- file.path("C:", "OneDrive", "OneDrive - ahdatalytics.com", "Clients", "Real Time Crime Index", "Open Source Data", "_Ben and Jeff work", "September 2024")

# Get a list of all CSV file paths within the directory (each source of data)
...
```{r}
# File path
pr_path <- "C:\\OneDrive\\OneDrive - ahdatalytics.com\\Clients\\Real Time Crime Index\\Open Source Data\\_Ben and Jeff work\\August 2024\\Puerto_Rico_Aggregated_Since_2017.csv"
...
# new state column to match the rest
pr_dat$State <- "PR"

pr_dat <- pr_dat %>%
  select(all_of(acceptable_names))

#remove months outside sample period
# Filter out observations with the current month and year or previous month and current year
pr_dat <- pr_dat %>%
  filter(
    !(Year == current_year & Month == current_month) &
...
```{r}
pr <- read.csv("C:\\Users\\daveh\\Downloads\\Crime for Major Cities (English) September2024.csv")
```
#clean up reshape
```{r}
...
write.csv(pr_counts, "C:\\OneDrive\\OneDrive - ahdatalytics.com\\Clients\\Real Time Crime Index\\Open Source Data\\_Ben and Jeff work\\September 2024\\Puerto_Rico_Aggregated_Since_2017.csv", row.names = FALSE)
