...
# Reorder columns
final_df1 <- final_df[, column_order]
```
convert NA's created to NR's because there is now a gap between an idealized time frame and when things were reported
```{r}
# Function to convert invalid numbers to "NR"
convert_invalid_to_NR <- function(x) {
  suppressWarnings({
    num_x <- as.numeric(x)
  })
  ifelse(is.na(num_x), "NR", x)
}

# Apply the function to columns 2 to 9 - crime count columns only
final_df1[, 2:9] <- lapply(final_df1[, 2:9], convert_invalid_to_NR)
```
...
final_df1$State <- state_value

```
format the columns and then we're really done.
```{r}
# Function to convert to numeric and handle invalid numbers
convert_to_numeric <- function(x) {
  suppressWarnings(as.numeric(x))
}

# Convert columns 2 to 11 to numeric
final_df1[, 2:11] <- lapply(final_df1[, 2:11], convert_to_numeric)

# Convert remaining columns to character
convert_to_character <- function(x) {
  suppressWarnings(as.character(x))
}
final_df1[, -c(2:11)] <- lapply(final_df1[, -c(2:11)], convert_to_character)
```
...
write.csv(final_df1, file = file_path, row.names = FALSE)
```
