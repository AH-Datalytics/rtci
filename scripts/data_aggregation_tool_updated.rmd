...
# Direct fix: use confirmed-correct nationwide_counts value for Nationwide Count rows
nw_pop_val <- unique(nationwide_counts$`Population Total`)[1]
message("=== Applying nationwide pop override: ", nw_pop_val)
...
message("=== Individual agency pop sum (CDE): ",
        sum(df_w_ref1$population[!df_w_ref1$`Agency Name` %in% aggregate_names], na.rm = TRUE) /
        length(unique(paste(df_w_ref1$Year[!df_w_ref1$`Agency Name` %in% aggregate_names],
                            df_w_ref1$Month[!df_w_ref1$`Agency Name` %in% aggregate_names]))))
