...
check_region <- ref_df2 %>%
  mutate(successful_merge = !is.na(region_name.x))
...
```{r}
cdf_vcpc_nats <- merge(cdf_vcpc_nats, ref_df2, by = c("city_state", "Agency_Type"), all.x = TRUE)
```
```{r}
...
#get the number of unique agencies across both agency types
...
region_vars <- region_vars %>% mutate(State = Region) %>% select(-Region)
```
