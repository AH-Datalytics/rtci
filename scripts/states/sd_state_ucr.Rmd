...
```{r}
data_dir <- "C:\\OneDrive\\OneDrive - ahdatalytics.com\\Clients\\Real Time Crime Index\\Open Source Data\\South Dakota\\State UCR"
```
yearly, incident level files
...
adjust_dataframes <- function(df, columns_to_keep) {
...
ldf3<- lapply(ldf2, adjust_dataframes, columns_to_keep = columns_to_keep)
...
#drop all records with NA in the month
final_df <- final_df[!is.na(final_df$Year),]
head(final_df)
```
add a column for State and then we're done.
```{r}
# Specify the value for the "State" column
state_value <- "South Dakota"
...
folder_path <- "C:\\OneDrive\\OneDrive - ahdatalytics.com\\Clients\\Real Time Crime Index\\Open Source Data\\South Dakota\\Formatted Data"

# Create the full file path
file_path <- file.path(folder_path, "sd_jan22_present.csv")
