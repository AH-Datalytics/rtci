...
onedrive_win <- file.path(Sys.getenv("USERPROFILE"), "OneDrive - ahdatalytics.com")
...
# Dynamically locate OneDrive directory
onedrive_mac <- "~/Library/CloudStorage/OneDrive-ahdatalytics.com"
onedrive_win <- file.path(Sys.getenv("OneDrive"), "OneDrive - ahdatalytics.com")
...
# Add new state column to match the rest
pr_dat$State <- "PR"
...
write it out!
```{r}
# TEMPORARILY COMMENTING THIS OUT SO IT DOESN'T WRITE TO THE ONEDRIVE WHILE IN PRACTICE MODE -- OSCAR
...
# file_path1 <- file.path(folder_path1, "sample_aug_2024.csv")
# write.csv(cleaned_data, file = file_path1, row.names = FALSE)


## OSCAR Write current sample to Github data/archive
# Extract the most recent date from the cleaned_data dataframe
latest_date <- max(cleaned_data$date, na.rm = TRUE)

# Format the date as "mon_yy" (e.g., "oct_24")
formatted_date <- tolower(format(latest_date, "%b_%y"))

# Construct the file path dynamically
output_file_path <- file.path(
  here("data", "archive"),
  paste0(formatted_date, "_agencies.csv")
)

sample_cities <- sample_cities %>% 
  mutate(date = formatted_date) %>% 
...
write.csv(sample_cities %>% select(city_state, date), output_file_path, row.names = FALSE)




# file_path3 <- file.path(folder_path1, "sample_cities_2024_09.csv" )
...
```{r}
pr <- read.csv("C:\\Users\\daveh\\Downloads\\Crime for Major Cities (English) October2024.csv")
```
#clean up reshape
```{r}
...
write.csv(pr_counts, "C:\\OneDrive\\OneDrive - ahdatalytics.com\\Clients\\Real Time Crime Index\\Open Source Data\\_Ben and Jeff work\\October 2024\\Puerto_Rico_Aggregated_Since_2017.csv", row.names = FALSE)
...
initial scrapes
```{r}
scrape_all<- read.csv("C:\\OneDrive\\OneDrive - ahdatalytics.com\\Clients\\Real Time Crime Index\\Data Auditing and Validation\\aggregated_since_2017.csv")
```