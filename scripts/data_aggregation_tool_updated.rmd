...
cdf_all <- read_csv("C:/OneDrive/OneDrive - ahdatalytics.com/Clients/Real Time Crime Index/Open Source Data/_Ben and Jeff work/Months/2026/042026 - Final.csv",
...
cdf_recent <- cdf_recent %>%
  mutate(city_state = paste(`Agency Name`, State, sep = ","))

cdf_recent <- cdf_recent %>%
  mutate(city_state_id = paste(city_state, Type, sep = ","))

#create Agency_Type
cdf_recent <- cdf_recent %>%
  mutate(Agency_Type = Type)
```
...
message("=== nationwide_counts Population Total (sample 3 months): ")
print(head(nationwide_counts[, c("Year", "Month", "Population Total")], 3))
...
df_w_ref<-df_w_ref %>% select(-State.y) %>% mutate(State = State.x, state_abbr) %>% select(-State.x)
```
...
message("=== df_w_ref1 columns containing 'population' (before line 775): ",
        paste(grep("population", names(df_w_ref1), ignore.case=TRUE, value=TRUE), collapse=", "))
message("=== df_w_ref1 'Population Total' for Nationwide Count rows: ",
        unique(df_w_ref1$`Population Total`[df_w_ref1$`Agency Name` == "Nationwide Count"]))
...
# Use CDE population_1 for individual agency rows so map covered population
# matches the nationwide total
aggregate_names <- c("Nationwide Count", "Regional Sample Counts",
                     "State Sample Counts", "<100k", "100k-250k", "250k-1mn", "1mn+")
df_w_ref1 <- df_w_ref1 %>%
  mutate(population = if_else(
    !`Agency Name` %in% aggregate_names & !is.na(population_1),
    as.numeric(population_1),
    as.numeric(population)
  ))

# Compute nationwide population from sample agencies only.
# nationwide_counts$`Population Total` includes non-sample agencies that pass
# the completeness filter; this recomputes from sample_cities.csv to match the map.
sample_city_ids_sc <- read_csv("../data/deprecated/sample_cities.csv", show_col_types = FALSE) %>%
  pull(city_state_id)
nw_pop_val <- df_w_ref1 %>%
  filter(city_state_id %in% sample_city_ids_sc, !`Agency Name` %in% aggregate_names) %>%
  distinct(city_state_id, population) %>%
  summarise(total = sum(population, na.rm = TRUE)) %>%
  pull(total)
message("=== Nationwide pop (sample agencies only): ", nw_pop_val)
df_w_ref1$population[df_w_ref1$`Agency Name` == "Nationwide Count"] <- nw_pop_val
...
write.csv(sample_cities_viz, "data/deprecated/sample_cities.csv", row.names = FALSE)
...
write.csv(cleaned_data, "data/deprecated/pre_processed.csv", row.names = FALSE)
