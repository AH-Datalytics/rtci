...
data_dir <- "C:\\OneDrive\\OneDrive - ahdatalytics.com\\Clients\\Real Time Crime Index\\Open Source Data\\_Ben and Jeff work\\Next Month's Update"
```
...
#change column names where they don't match - larceny to theft - this seems to be a consistent refrain
...
# apply the function and take a look at the column names real quick
...
# get rid of the resulting row.names - thank you very much
...
group_100k <- nationwide_counts %>% filter(Population_Group == "<100k")
group_100k_250k <- nationwide_counts %>% filter(Population_Group == "100k-250k")
group_250k_1mil <- nationwide_counts %>% filter(Population_Group == "250k-1mi")
group_1mil <- nationwide_counts %>% filter(Population_Group == "1mi+")
```
add the nationwide counts to the sample for the viz
```{r}
# Add values for agency name field so we can bind
