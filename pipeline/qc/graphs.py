    ...
    def run(self):
        ...
        for ori in ["CT0001500"]:
            ...
            for crime in self.crimes:
                ...
                #
                # # generate figure fig = px.line( to_graph, x="Date", y=crime, color="Source", color_discrete_map={
                # "final_sample.csv": "#2D5EF9", "scraper": "red", }, labels={ crime: f"Reported {crime.rstrip('y') +
                # 'ie' if crime.endswith('y') else crime}s Per Month" }, line_shape="spline", ) fig.update_traces(
                # mode="lines+markers") fig.update_traces(line=dict(width=2.25)) fig.update_traces(marker=dict(
                # size=5)) fig.update_layout( title=f"Show me reported <b style='color:#2D5EF9'>{crime.rstrip('y') +
                # 'ie' if crime.endswith('y') else crime}s</b> in <b style='color:#2D5EF9'>{us.states.lookup(
                # to_graph['State'].unique()[0]).name}</b> for <b style='color:#2D5EF9'>{to_graph['Agency
                # Name'].unique()[0]}</b> as <b style='color:#2D5EF9'>Monthly Totals</b>" )
                #
                # if self.args.test: fig.show() else: # save figure to s3 snapshot_fig( logger=self.logger, fig=fig,
                # path=self.prefix, filename=f"{to_graph['State'].unique()[0].lower()}/{to_graph['Agency
                # Name'].unique()[0].lower().replace(' ', '_')}/{crime.lower().replace(' ', '_')}", )
