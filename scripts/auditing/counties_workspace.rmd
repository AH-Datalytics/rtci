...
head(county_ldf[[1]])
```
get info on all the excels
```{r}
# Function to get info on the first sheet of each Excel file
get_excel_info <- function(file) {
...
#file_paths <- list.files(path = "path/to/excel/files", pattern = "\\.xlsx$", full.names = TRUE)

# Apply the function to each Excel file and combine the results into a single dataframe
excel_info_df <- map_df(file_paths, get_excel_info)
...
acceptable_names <- c("Agency Name", "Murder", "Rape", "Robbery", "Aggravated Assault",  
                      "Burglary", "Theft", "Motor Vehicle Theft", "Year", "Month", "State", "Agency_Type")
