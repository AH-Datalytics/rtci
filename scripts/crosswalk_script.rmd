...
aggregated     <- read_csv("C:/Users/daveh/Downloads/cde_data_since_1985 (1).csv", show_col_types = FALSE)
...
new_cde_agencies <- read_csv(
  "C:/Users/daveh/Downloads/check_agencies - check_agencies.csv",
  show_col_types = FALSE
)

cde_data_all <- read_csv(
  "C:/Users/daveh/Downloads/cde_data_since_1985 (1).csv",
  show_col_types = FALSE
)

# 2) From new_cde_agencies, keep every unique ORI where include_yn == Y/y
new_cde_oris <- new_cde_agencies %>%
  mutate(include_yn = str_to_lower(str_trim(include_yn))) %>%
  filter(include_yn == "y") %>%
  distinct(ori) %>%                     # keep one row per ORI
  filter(!is.na(ori) & ori != "")       # drop blanks just in case
