...
#create a new unique id for cdf_vcpc to fix join issue
cdf_vcpc <- cdf_vcpc %>%
  mutate(city_state_id = paste(city_state, Agency_Type, sep = ","))
...
cdf_vcpc <- cdf_vcpc %>% select(city_state, vio_crime, prop_crime, Year, Month, city_state_id)
```
...
START HERE ON MONDAY 3/17 - almost done checking!
...
join ref to sample
```{r}
df_w_ref <- df_mvs_12mo1 %>% select(-city_state.x, -city_state.y) %>%
  left_join(ref_df1, by = "city_state", "Agency_Type")
```
