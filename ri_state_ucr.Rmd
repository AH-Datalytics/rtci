...
```{r}
# Single list of columns to be used for combinations
combination_cols <- c("A", "B", "C", "D")

# Function to get all combinations of three columns from the specified list of columns
get_combinations <- function(df, cols) {
  # Get all combinations of three columns from the specified list
  comb_cols <- combn(cols, 3, simplify = FALSE)
  
  # Map over each combination and select these columns, then drop duplicates
  map_df(comb_cols, ~ df %>%
           select(all_of(.x)) %>%
           distinct())
}

# Generate combinations for the single DataFrame
combinations <- get_combinations(df, combination_cols)

# For demonstration, let's assume we have an existing dataframe to join with
existing_df <- data.frame(A = 1:3, B = 7:9, C = 13:15, X = 101:103)

# Join combinations with the existing dataframe and fill NA values with 0
# Assuming the columns to join on are known and present in both dataframes
result_df <- full_join(combinations, existing_df, by = c("A", "B", "C")) %>%
             replace_na(list(X = 0))  # Adjust this to the columns you expect NAs
