...
# generate a new date field based on month and year
ucr_data <- ucr_data %>%
  mutate(Date = as.Date(paste(Year, Month, "01", sep = "-"), "%Y-%m-%d"))

# Drop the columns Year, Month, Months.Missing
ucr_long <- ucr_data %>%
  select(-Year, -Month, -Months.Missing, -LONG, -LAT, -Last.Report.Month)

# Reshape to long based on crime type
ucr_long <- ucr_long %>%
  pivot_longer(cols = c("Murder", "Rape", "Robbery", 
                        "Agg.Assault", "Burglary", "Larceny", "MVT", "Arson"),
                            names_to = "Crime Type",
...
head(ucr_long)                           
```
break it up between wide and long shaping
```{r}
# Reshape to wide format with Date as columns
ucr_wide <- ucr_long %>%
  pivot_wider(names_from = Date, values_from = Count)

# New column to match on
ucr_wide$City <- ucr_wide$Agency.Name
