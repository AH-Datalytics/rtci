...
```{r}
# Function to convert character columns to numeric where applicable, excluding specific columns
convert_to_numeric <- function(df, exclude_cols) {
  # Select columns to be converted
  cols_to_convert <- setdiff(names(df), exclude_cols)
...
exclude_cols <- c("State", "Agency.Name")

# Apply the conversion function to each data frame in the list
ed1 <- lapply(extracted_data, convert_to_numeric, exclude_cols)

# Combine data frames with missing columns filled with NA
combined_df <- bind_rows(ed1)
