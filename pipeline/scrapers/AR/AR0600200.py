...
class AR0600200(Scraper):
    ...
    def scrape(self):
        ...
        print(pages)
        doc = parse_pdf(self, url=None, pages=pages)
        ...
        for page in doc.pages:
            ...
            for table in tables:
                ...
                rows = [
                    [cell.text.strip() for cell in row.cells] for row in table.rows[2:]
                ]
                df = pd.DataFrame(rows, columns=headers)
                dfs.append(df)

        df = pd.concat(dfs)
        df.to_csv(f"test_p_{pages[0]}-{pages[-1]}.csv", index=False)
