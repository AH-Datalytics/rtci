...
#create city_state_id
counties_cdf <- counties_cdf %>% mutate(city_state_id = paste(`city_state`, `Agency_Type`, sep = ","))
```
let's check for counties that don't connect to ref file
```{r}
# Perform the merge
counties_cdf_test <- merge(counties_cdf, ref_df1, by = "city_state_id")
...
#clean up columns
counties_cdf_test <- counties_cdf_test %>% mutate(State = State.x, city_state = city_state.x, Agency_Type = Agency_Type.x) %>%
select(-State.y, -city_state.y, -Agency_Type.y, -State.x, -city_state.x, -Agency_Type.x)

names(counties_cdf_test)
```
