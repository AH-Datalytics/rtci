...
data_dir <- "C:\\OneDrive\\OneDrive - ahdatalytics.com\\Clients\\Real Time Crime Index\\Open Source Data\\_Ben and Jeff work\\July 2024"
```
...
#clean up reshape
```{r}
# Create a complete combination of municipalities, years, and months
complete_data <- pr %>%
  distinct(Date_Date_Year, Date_Date_Month, Municipalities_Name_1) %>%
  expand(Municipalities_Name_1, Date_Date_Year, Date_Date_Month)

# Join the original dataset to this complete grid
merged_data <- complete_data %>%
  left_join(pr, by = c("Municipalities_Name_1", "Date_Date_Year", "Date_Date_Month"))

# Pivot PBI_CRIMEN_codigo into columns, filling in missing values with 0
pivoted_data <- merged_data %>%
  pivot_wider(names_from = PBI_CRIMEN_codigo, 
              values_from = PBI_CRIMEN_Crime_Count, 
              values_fill = list(PBI_CRIMEN_Crime_Count = 0))

head(pivoted_data)

# Create a table summarizing the count of each crime type
crime_counts <- pr %>%
  group_by(PBI_CRIMEN_codigo) %>%
  summarise(total_count = sum(PBI_CRIMEN_Crime_Count, na.rm = TRUE))
