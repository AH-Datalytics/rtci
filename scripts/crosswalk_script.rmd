...
```
read cde population data from github
```{r}
# Read CDE agency-level population data
cde_pop_df <- read.csv("https://raw.githubusercontent.com/AH-Datalytics/cde-population/master/cde_populations.csv") %>%
  mutate(ori = toupper(ori)) %>%
  rename(population_1 = population) %>%
  select(ori, population_1)
```
...
fbi_participation_2023_updated <- fbi_participation_2023_updated %>%
  mutate(
    pub_agency_name = case_when(
      pub_agency_name == "Port St. Lucie" ~ "Port St Lucie",
      TRUE ~ pub_agency_name
    ),
    city_state_id = paste(pub_agency_name, state_abbr, agency_type_name, sep = ",")
  )
```
merge cde population to fbi participation based on ori
```{r}
library(dplyr)
...
# ---- LEFT JOIN: keep all fbi participation records, bring in cde population_1 ----
merged_df_left <- fbi_participation_2023_updated %>%
  left_join(cde_pop_df, by = "ori")

# ---- Quick checks ----
message("Merged rows: ", nrow(merged_df_left))
message("Matched with CDE pop: ", sum(!is.na(merged_df_left$population_1)))
message("Unmatched (no CDE pop): ", sum(is.na(merged_df_left$population_1)))
...
#update population_1 with fbi participation pop as fallback where cde has no match
