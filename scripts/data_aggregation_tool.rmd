...
ref_df1 <- ref_df %>%
  left_join(cities_counties_ref, by = c("city_state" = "city_state", "Agency_Type" = "agency_type_name"))
...
ref_df1 <- ref_df1 %>% select(-Agency_Type.y, -State)
...
cdf_vcpc_nats <- merge(
  ...
  by.y = c("city_state", "Agency_Type"), 
  all.x = TRUE
)
cdf_vcpc_nats1 <- cdf_vcpc_nats %>% mutate(State_ref = State) %>%   select(-any_of(c("State.x", "State.y")))
```
check to make sure the regions are attached
```{r}
# Filter records where region_name is NA
na_region_records <- cdf_vcpc_nats1 %>%
...
nationwide_counts <- cdf_vcpc_nats_usa %>%
  ...
  summarise(
    ...
    Agency_num = n_distinct(paste(city_state, Agency_Type, sep = "_")), # Count unique agencies per month
    .groups = 'drop'
  )
...
df_w_ref <- df_mvs_12mo1 %>%
  left_join(ref_df1, by = "city_state")
```
