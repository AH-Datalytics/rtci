...
duplicates <- combined_df %>%
  group_by(`Agency Name`, State, Year, Month) %>%
  filter(n() > 1) %>%
  ungroup()
...
# Optional: count how many duplicate groups
dup_counts <- duplicates %>%
  count(`Agency Name`, State, Year, Month, name = "duplicate_count")
...
cdf_vcpc_nats <- cdf_vcpc_nats %>%
  mutate(
    ...
    population = case_when(
      ...
      city_state_id == "St Charles,MO,County" ~ 119730,
      city_state_id == "St Louis,MO,County" ~ 398969,
      TRUE ~ population_1  # Retain existing values for other rows
    ),
