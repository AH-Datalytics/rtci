...
class TestDateApi(TestCommonAdapter):
    ...
    async def test_date_range_retriever(self):
        ...
        response = await bot.resolve_dates(f"Which city had the highest number of murders during the date range provided?")
        ...
        self.assertIsNone(response)
        response = await bot.resolve_dates(f"What is the percent change in motor vehicle theft from 2023 to 2024?")
        self.assertIsNotNone(response)
        self.assertEqual(response.start_date.year, 2023)
        self.assertEqual(response.end_date.year, 2024)
