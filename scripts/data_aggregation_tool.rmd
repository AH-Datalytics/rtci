...
```{r}
# Concatenate pub_agency_name and state_abbr to create "city_state"
region_df <- fbi_participation_2023 %>%
  mutate(city_state = paste(pub_agency_name, state_abbr, sep = ", ")) %>%
  select(city_state, region_name, division_name) %>%
  distinct(city_state, .keep_all = TRUE)

# Merge with ref_df by "city_state"
ref_df2 <- ref_df1 %>%
  left_join(region_df, by = "city_state")
...
```
why are there different records post-join?
```{r}
library(dplyr)
...
```{r}
cdf_vcpc_nats <- merge(cdf_vcpc_nats, ref_df2, by = "city_state", all.x = TRUE)
```
