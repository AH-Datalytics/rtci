...
```{r}
# Define the base OneDrive path
onedrive_mac <- file.path("~", "Library", "CloudStorage", "OneDrive-YourOrganizationName")
onedrive_win <- file.path("C:", "OneDrive", "OneDrive - ahdatalytics.com")

# Check which OneDrive directory exists
if (dir.exists(onedrive_mac)) {
  onedrive_dir <- normalizePath(onedrive_mac)
} else if (dir.exists(onedrive_win)) {
  onedrive_dir <- normalizePath(onedrive_win)
...
file_path <- file.path(sample_write_path, "final_sample.csv")
...
#sample cities sent to data folder for metadata viz
write.csv(sample_cities, "../data/sample_cities.csv", row.names = FALSE)

# copies with month labeled for archive/future auditing
month_suffix <- paste0("_", two_months_previous_month)

write.csv(
  undesirables_na_current,
  file.path("../scripts/auditing", paste0("missing_current", month_suffix, ".csv")),
  row.names = FALSE
)

write.csv(
  undesirables_na_historical,
  file.path("../scripts/auditing", paste0("missing_historical", month_suffix, ".csv")),
  row.names = FALSE
)

write.csv(
  undesirables_recency,
  file.path("../scripts/auditing", paste0("no_reported_data_past_two_months", month_suffix, ".csv")),
  row.names = FALSE
)

write.csv(
  remove_agencies,
  file.path("../scripts/auditing", paste0("agencies_removed_from_sample", month_suffix, ".csv")),
  row.names = FALSE
)

write.csv(
  sample_cities,
  file.path("../scripts/auditing", paste0("sample_cities", month_suffix, ".csv")),
  row.names = FALSE
)
```