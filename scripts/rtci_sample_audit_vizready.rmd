...
write.csv(cdf1, file = file_path, row.names = FALSE)
```
...
# Write the data frame to a .csv file
write.csv(cdf1, "..\\rtci\\deploy\\shiny_web_app\\sample_2018_present_wlinks.csv", row.names = FALSE)
```
...
priork_results <-ytd_poisson(samp2)
...
priork_results <-metrics_allpd(samp2)
...
priork_results <-priork_poisson(samp2)
```
