...
# Function to keep specified columns and order columns for renaming
...
# Apply the function to each dataframe in the list
ldf4 <- lapply(ldf3_num, keep_columns)
```
...
ldf6 <- lapply(ldf5, trim_agency_na)
```
...
ldf7 <- lapply(ldf6, replace_na)

head(ldf7[1])
```
row bind the list together
```{r}
combined_df <- do.call(rbind, ldf7)
```
